...
公开仓库版本默认不内置页面登录口令，打开后可直接进入配置页；真实平台账号密码请自行填写到本地 `automation/config.json` 或页面表单里。

在这个页面里：

- 点击“保存”只会把配置保存到当前浏览器
- 点击“开始执行”才会把当前配置写入 `automation/config.json` 并启动本地执行器

如果你是直接双击打开根目录里的 `index.html`，那属于离线配置模式：可以维护配置，但不会连接本地执行器。
